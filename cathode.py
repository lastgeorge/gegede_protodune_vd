...
class CathodeBuilder(gegede.builder.Builder):
    ...
    def configure(self, 
                 ...
                 widthCathode = Q('670.0cm'),
                 lengthCathode = Q('670.0cm'),
    ...
    def construct(self, geom):
        ...
        # Create void box for subtraction (slightly larger in x to ensure clean subtraction)
        void_box = geom.shapes.Box(
            self.name + "_void",
            dx=self.height/2 + Q('0.5cm'), # Extra 0.5cm as in PERL
            dy=self.void_width/2,
            dz=self.void_length/2)

        # For each void position, place a mesh grid
        # Define the void centers
        void_positions = [
            # First row (bottom)
        ...
        grid_positions = [
            [-0.25*self.width, -0.25*self.length],
            [-0.25*self.width, 0.25*self.length],
            [0.25*self.width, -0.25*self.length],
            [0.25*self.width, 0.25*self.length]
        ]

        # Create cathode frame through sequential boolean operations
        shape = cathode_box
        for j, (grid_y, grid_z) in enumerate(grid_positions):
            for i, (void_y, void_z) in enumerate(void_positions):
                shape = geom.shapes.Boolean(
                self.name + f"_shape{i+1}_{j+1}",
                type='subtraction',
                first=shape,
                second=void_box,
                pos=geom.structure.Position(
                    self.name + f"_void_pos{i+1}_{j+1}",
                    x=Q('0cm'),
                    y=void_y + grid_y,
                    z=void_z + grid_z))
    ...
    def place_in_volume(self, geom, volume, argon_dim, params):
        ...
        # Calculate base cathode position
        cathode_x = argon_dim[0]/2 - params['gas_argon_height'] - \
                   params['upper_xLArBuffer'] - \
                   (params['drift_active'] + params['readout_plane']) - \
                   self.height/2
                   
        cathode_y = -argon_dim[1]/2 + params['y_lar_buffer'] + self.width/2
        cathode_z = -argon_dim[2]/2 + params['z_lar_buffer'] + self.length/2

        cathode_vol = self.get_volume()
        
        # Place cathodes in 2x2 grid
        # for i in range(1):  # along y
        #     for j in range(1):  # along z
        #         idx = i*2 + j
                
        idx = 0
        i=0
        j=0
        # Create position for this cathode module
        pos = geom.structure.Position(
            f"{self.name}_pos_{idx}",
            x=cathode_x,
            y=cathode_y + i*self.width,
            z=cathode_z + j*self.length
        )
        
        # Create placement
        place = geom.structure.Placement(
            f"{self.name}_place_{idx}",
            volume=cathode_vol,
            pos=pos
        )
